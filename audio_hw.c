...
#define USI_LOGFUNC(...)  (ALOGV(__VA_ARGS__))
...
#define MIXER_SIDETONE_EQUALIZER	    "Sidetone Equalizer"
#define MIXER_AMIC_EQUALIZER		    "AMIC Equalizer"
#define MIXER_DMIC_EQUALIZER		    "DMIC Equalizer"
...
#define MIXER_AUXR_ENABLE_SWITCH            "Aux Right Playback Switch"
#define MIXER_AUXL_ENABLE_SWITCH            "Aux Left Playback Switch"
...
/* MSI: VOLTRON BEGIN */
/*Increased Digital mic volume*/
#define CAPTURE_DIGITAL_MIC_VOLUME            29
/* MSI: VOLTRON END */
...
/* MSI: VOLTRON BEGIN */
/* Setting Voice Record Uplink & Downlink Volumes */
...
#define PRODUCT_DEVICE_MC40   "MC40"
#define PRODUCT_DEVICE_BLUEPOINT	"bluepoint"
/* fm */
...
/*      .intval = DB_TO_CAPTURE_PREAMPLIFIER_VOLUME(0),	*/
        .intval = DB_TO_CAPTURE_PREAMPLIFIER_VOLUME(30),
/*USI--*/
    },
    {
...
struct mixer_ctls
{
    ...
    //+USI: castor use aux as output
    struct mixer_ctl *auxr_enable;
    struct mixer_ctl *auxl_enable;
};
...
/*USI++*/
    { /* Force to use OMAP4_BLAZE for MPA3 */
           adev->board_type = OMAP4_BLAZE;
           adev->sidetone_capture = 1;
    }
        /*return -EINVAL;*/
                              ...
                              int enable)
{
    ...
    unsigned int usi_i, usi_num_enums;
    char* usi_string;
    USI_LOGFUNC("++++++++++++ USI ++++++++++++");
    USI_LOGFUNC("In %s:", __FUNCTION__);
    /* Go through the route array and set each value */
    i = 0;
    while (route[i].ctl_name) {
        ctl = mixer_get_ctl_by_name(mixer, route[i].ctl_name);
        if (!ctl){
            USI_LOGFUNC("Can't find mixer_ctl \"%s\"", route[i].ctl_name);
            USI_LOGFUNC("------------ USI ------------");
            return -EINVAL;
        }
        if (route[i].strval) {
            usi_num_enums = mixer_ctl_get_num_enums(ctl);
            for(usi_i=0; usi_i < usi_num_enums; usi_i++){
              if(mixer_ctl_get_value(ctl, 0) == (int)usi_i){
                usi_string = mixer_ctl_get_enum_string(ctl, usi_i);
                break;
              }
            }
            USI_LOGFUNC("Before: %s = %s", mixer_ctl_get_name(ctl), usi_string);
            if (enable)
            ...
            USI_LOGFUNC("After : %s = %s", mixer_ctl_get_name(ctl), usi_string);
            //-USI debug
        } else {
            /* This ensures multiple (i.e. stereo) values are set jointly */
            for (j = 0; j < mixer_ctl_get_num_values(ctl); j++) {
                USI_LOGFUNC("Before: %s = %d", mixer_ctl_get_name(ctl), mixer_ctl_get_value(ctl, j));
                if (enable)
                    mixer_ctl_set_value(ctl, j, route[i].intval);
                else
                    mixer_ctl_set_value(ctl, j, 0);
                USI_LOGFUNC("After : %s = %d", mixer_ctl_get_name(ctl), mixer_ctl_get_value(ctl, j));
            }
        }
        i++;
    }
...
static void set_eq_filter(struct omap_audio_device *adev)
{
    ...
    int dl1_eq_applicable = adev->devices & (AUDIO_DEVICE_OUT_WIRED_HEADSET | AUDIO_DEVICE_OUT_WIRED_HEADPHONE | AUDIO_DEVICE_OUT_EARPIECE);

    if (!adev->dl2_support)
        dl1_eq_applicable |= adev->devices & AUDIO_DEVICE_OUT_SPEAKER;

    /* 4Khz LPF is used only in NB-AMR voicecall */
    if ((adev->mode == AUDIO_MODE_IN_CALL) && dl1_eq_applicable && (adev->tty_mode == TTY_MODE_OFF) && !adev->wb_amr){
        usi_num_enums = mixer_ctl_get_num_enums(adev->mixer_ctls.dl1_eq);
        for(usi_i=0; usi_i < usi_num_enums; usi_i++){
          if(mixer_ctl_get_value(adev->mixer_ctls.dl1_eq, 0) == (int)usi_i){
            usi_string = mixer_ctl_get_enum_string(adev->mixer_ctls.dl1_eq, usi_i);
            break;
          }
        }
        USI_LOGFUNC("Before: %s = %s", mixer_ctl_get_name(adev->mixer_ctls.dl1_eq), usi_string);
        ...
        USI_LOGFUNC("After : %s = %s", mixer_ctl_get_name(adev->mixer_ctls.dl1_eq), usi_string);
                              ...
                              int headset_mic_on, int sub_mic_on)
{
    ...
    if (adev->mode == AUDIO_MODE_IN_CALL) {
        ...
        case AUDIO_SOURCE_MIC: /* general capture */
	       ...
	       (adev->board_type == OMAP5_PANDA)) {
/* MSI: VOLTRON BEGIN */
/* Setting Digital microphone volume */
    ...
    struct omap_stream_out *out)
{
    ...
    if (out) {
        if (mm_stream) {
            if (adev->dl2_support){
                USI_LOGFUNC("Before: %s = %d",
                    mixer_ctl_get_name(adev->mixer_ctls.mm_dl2),
                    mixer_ctl_get_value(adev->mixer_ctls.mm_dl2, 0));
                mixer_ctl_set_value(adev->mixer_ctls.mm_dl2, 0, set_speaker_on);
                USI_LOGFUNC("After : %s = %d",
                    mixer_ctl_get_name(adev->mixer_ctls.mm_dl2),
                    mixer_ctl_get_value(adev->mixer_ctls.mm_dl2, 0));
            }
            USI_LOGFUNC("Before: %s = %d",
                mixer_ctl_get_name(adev->mixer_ctls.mm_dl1),
                mixer_ctl_get_value(adev->mixer_ctls.mm_dl1, 0));
            ...
            if (adev->dl2_support){
                USI_LOGFUNC("Before: %s = %d",
                    mixer_ctl_get_name(adev->mixer_ctls.tones_dl2),
                    mixer_ctl_get_value(adev->mixer_ctls.tones_dl2, 0));
            ...
            USI_LOGFUNC("Before: %s = %d",
                mixer_ctl_get_name(adev->mixer_ctls.tones_dl1),
                mixer_ctl_get_value(adev->mixer_ctls.tones_dl1, 0));
            mixer_ctl_set_value(adev->mixer_ctls.tones_dl1, 0, set_dl1_on);
            USI_LOGFUNC("After : %s = %d", mixer_ctl_get_name(adev->mixer_ctls.tones_dl1), mixer_ctl_get_value(adev->mixer_ctls.tones_dl1, 0));
        }
    }
	...
	// castor need to turn on aux
    if ((adev->board_type == OMAP5_PANDA) || (adev->board_type == OMAP4_BLAZE)){
...
/* MSI: VOLTRON BEGIN */
/* Setting dmic0 for audio record case */
                     ...
                     hw_device_t** device)
{
    ...
    adev->mixer_ctls.auxr_enable = mixer_get_ctl_by_name(adev->mixer,
                                           MIXER_AUXR_ENABLE_SWITCH);
    adev->mixer_ctls.auxl_enable = mixer_get_ctl_by_name(adev->mixer,
                                           MIXER_AUXL_ENABLE_SWITCH);
